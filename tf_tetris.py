...
while True:
    ...
    for j in range(max_ep):
        ...
        if done:
            ...
            its_time_to_update_weights = ((game_num % update_frequency) == 0
                                          and (game_num != 0))
            if its_time_to_update_weights:
                print game_num, '{:2.2f}'.format(
                    np.mean(game_rewards[game_num -
                                         update_frequency:game_num]))
                all_ep_history = np.vstack(all_ep_history)
                feed_dict = {
                    rewards: all_ep_history[:, 2],
